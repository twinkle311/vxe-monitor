...
- 🔋 **托盘数字电量图标**:任务栏图标内嵌实时电量百分比,电量条分 3 档颜色预警(>20% 绿 / 11~20% 黄 / ≤10% 红),充电时附加 ⚡ 标志。
- 🖱️ **左键 OSD 悬浮提示**:单击托盘图标弹出电量 / 型号 / 连接方式 / 电压面板,1.6 秒后平滑淡出。
- 📋 **右键菜单详情**:电池电量、充放电状态、电池电压 (mV)、连接方式(2.4G 1K/2K/4K/8K、有线、Nearlink)、固件版本一目了然。
- 🚀 **开机自启动**:右键菜单一键切换(基于 `HKCU` 注册表,无需管理员权限)。
- ⚡ **极简轻量**:纯 Win32 API,单个 exe,无第三方运行时依赖。
- 🔌 **自动重连**:接收器插拔 / 鼠标开关机自动感知,无需手动干预。
